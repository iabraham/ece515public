...
# ╔═╡ 9d0f3178-65a2-11f0-3fae-4f016bc78b09
# ╠═╡ show_logs = false
begin
...
# ╔═╡ 0bf5b742-b6e4-41e8-a452-bec3fc0444d2
md"""
**Author:** [itabrah2@illinois.edu](https://ivan.web.illinois.edu)

**Note:** Parts of notebook built with input (and a fair bit of debugging) from Google Gemini's version `Pro 2.5`: See [here](https://courses.grainger.illinois.edu/ece515/fa2025/projectdemo/gemini_lqr.html).

See source of this notebook [here](https://courses.grainger.illinois.edu/ece515/fa2025/_projectsource/lqr.jl).
"""

# ╔═╡ 574e5d38-1c60-4ac9-b955-1ba5898c3e40
md"""## Extra credit project demo
The following was my prompt to Google's Gemini. 
"""

# ╔═╡ ba2dbb95-5a99-4fb8-af1d-c0ebcba271ad
md"""
>  I want to create a simple interactive notebook to show students the relation between Q, R and N matrices and pendulum dynamics where the objective is to stabilize an inverted pendulum using LQR control. I want to do this in Julia using Pluto notebooks and the Pluto UI sliders. Please help. The notebook should have three sections -- (a) derivations of EOM using the Symbolics.jl package followed by linearization, (b) implementation of LQR control and (c) interactive part to vary LQR gains and see change in system response.
"""

# ╔═╡ 849ee024-6026-4bee-8a38-e06942176c42
md""" See its output [here](https://courses.grainger.illinois.edu/ece515/fa2025/projectdemo/gemini_lqr.html). The output looks messy, but about 60% of the content of this page is actually there in that output. For comparison, to see the **source code** of this notebook, [click here](https://courses.grainger.illinois.edu/ece515/fa2025/_projectsource/lqr.jl). 

### Tasks 

1. Analyze this notebook for accuracy/mistakes (GenAI can famously make mistakes).
2. Submit a 1 page proposal by **October 17**, on what concept/topic you would like to create an applet/demo. Include in this proposal the issues you find with _this_ notebook. 
3. Submit a **_complete_** Github repository that anyone can clone and run your applet from by last day of class. 

More details can be found on the [Course Website](https://courses.grainger.illinois.edu/ece515/fa2025/extracred.html). 

##### You are free to use GenAI and Python/Matlab/Julia/Octave/Java/etc. for your project! 
"""

# ╔═╡ 88ed3cfe-ace9-402c-bd78-79cb08a95846
md""" ## LQR Demonstration using Inverted Pendulum """

# ╔═╡ d5c8add0-154c-42f3-b7f7-f01a9871202c
md"""
### (a) 📜 Derivation of Equations of Motion

In this section, we derive the system's equations of motion (EOM) using Lagrangian mechanics. The [`Symbolics.jl`](https://symbolics.juliasymbols.org/stable/) package allows us to perform these derivations automatically.

The system consists of a cart of mass $M$ and a pendulum of mass $m$ with length $L$. The states are the cart's position $x$ and the pendulum's angle $\theta$ (where $\theta=0$ is the upright position). A force $u$ (denoted as $F$ in diagram below) is applied to the cart.
"""

# ╔═╡ 29532b07-eb15-44e2-915f-d56ce7b8b89f
Resource("https://latexdraw.com/wp-content/uploads/2021/04/Inverted-Pendulum-in-TikZ.png", :width => 400)

# ╔═╡ 61215879-e222-4003-ad61-bd15546bb807
md"""(**Source:** Image from latexdraw.com)"""

# ╔═╡ c4dc32a0-271d-4843-9d82-182b90a97b39
md"""
### Symbolic Variables

First, we define all symbolic variables for our system parameters, states, and their time derivatives.
"""

# ╔═╡ dc52a40a-5c8b-4e23-9f3f-568914e005c2
begin
	@variables t                           # time
	@variables m M L g                     # system parameters
	@variables x(t) θ(t) u(t)              # states and input
	
	# Define differential operator
	D = Differential(t)
	
	# Display variables nicely in LaTeX
	(t, m, M, L, g, x, θ, u, D)
end

# ╔═╡ f23034eb-1030-42a3-bbf8-a58f6b78656b
md"""
### Lagrangian Mechanics

We use a Lagrangian mechanics approach to find the EOMs. The Lagrangian $\mathcal{L}$ is defined as the difference between the system's kinetic energy ($T$) and potential energy ($V$).

$\mathcal{L} = T - V$

1.  **Kinetic Energy ($T$)**: This is the sum of the kinetic energy of the cart and the pendulum bob.
    * $T_{cart} = \frac{1}{2} M (\dot{x})^2$
    * $T_{pendulum} = \frac{1}{2} m (v_{px}^2 + v_{py}^2)$
    where $v_{px} =\dfrac{d}{dt}(x_p), v_{py} = \dfrac{d}{dt}(y_p), x_p = x + L \sin(\theta)$ and $y_p = L \cos(\theta)$.

2.  **Potential Energy ($V$)**: This is determined by the height of the pendulum bob.
    * $V = m g L \cos(\theta)$

The equations of motion are then found using the Euler-Lagrange equation for each generalized coordinate ($q_i \in \{x, \theta\}$), including the external force $u$ applied to $x$:

$\frac{d}{dt} \frac{\partial \mathcal{L}}{\partial \dot{q_i}} - \frac{\partial \mathcal{L}}{\partial q_i} = F_{q_i}$
"""

# ╔═╡ aa153512-bcc7-4312-8fdb-fecb92509ed0
begin
	...
	# Generalized coordinates q = [x, θ]
	# Forces F = [u, 0]
	EL_x = D(Symbolics.gradient(Lag, [D(x)])[1]) - Symbolics.gradient(Lag, [x])[1] ~ u
	EL_θ = D(Symbolics.gradient(Lag, [D(θ)])[1]) - Symbolics.gradient(Lag, [θ])[1] ~ 0
end;

# ╔═╡ d1502ab0-4d4d-4cf3-ac11-b795cde1c7f1
md"""
### Solving for Accelerations

The equations above are implicit. To get our standard state-space form, we need to solve for the accelerations $\ddot{x}$ and $\ddot{\theta}$ -- but first we ask the Symbolics package to expand/compute the derivatives in the expression above (it [evaluates lazily](https://en.wikipedia.org/wiki/Lazy_evaluation)).
"""

# ╔═╡ 57b8ca24-63c9-4898-98ac-d55b20f622d7
begin
	# Solve for the second derivatives (accelerations)
	eqs = expand_derivatives.([EL_x, EL_θ])
	sol = Symbolics.symbolic_linear_solve(eqs, [D(D(x)), D(D(θ))], check=true)
end;

# ╔═╡ e6c88d0c-7c55-424d-9406-12cdb16c7e0b
x_dd_expr = sol[1]

# ╔═╡ 6180463d-b404-4305-ac93-6b9f68190900
θ_dd_expr = sol[2]

# ╔═╡ 02b262cb-5d65-4664-8b99-29ee9116ac90
md"""**Exercise** -- Verify the above are correct!"""

# ╔═╡ 76c22e01-c3f8-4685-aca6-64386cfa1527
md"""
## (b) 🤖  System Linearization

The LQR controller requires a linear time-invariant (LTI) model of the form $\dot{\mathbf{z}} = A\mathbf{z} + B\mathbf{u}$. We linearize our nonlinear system around the unstable equilibrium point: pendulum perfectly upright and stationary at the origin.

The state vector is $\mathbf{z} = [x, \dot{x}, \theta, \dot{\theta}]^T$.
The equilibrium point is $\mathbf{z}_{eq} = [0, 0, 0, 0]^T$ and $u_{eq} = 0$.

The matrices $A$ and $B$ are the Jacobians of the system dynamics with respect to the state and input, evaluated at the equilibrium point.

$A = \frac{\partial f}{\partial \mathbf{z}} \bigg|_{\mathbf{z}_{eq}, u_{eq}}$ and 

$B = \frac{\partial f}{\partial u} \bigg|_{\mathbf{z}_{eq}, u_{eq}}$
"""

# ╔═╡ 7801aa9b-6833-4f09-813c-39cff6fb3004
begin
	# Define the symbolic state vector and nonlinear dynamics function f
	z = [x, D(x), θ, D(θ)]
	f_sym = [
		D(x),
		x_dd_expr,
	...
	A_sym = Symbolics.jacobian(f_sym, z)
	B_sym = Symbolics.jacobian(f_sym, [u])

	# Define the equilibrium point
	eq_point = Dict(x => 0, D(x) => 0, θ => 0, D(θ) => 0, u => 0)

	# Substitute the equilibrium point values into the Jacobians
	A_eq = substitute(A_sym, eq_point)
	B_eq = substitute(B_sym, eq_point)
end;

# ╔═╡ dfb9f46e-110f-476f-b0af-09c3d9b46374
md"""
## (c) ⚙️ LQR Controller Implementation

The Linear-Quadratic Regulator (LQR) is an optimal control strategy. It finds the feedback control law $u = -K\mathbf{z}$ that minimizes a quadratic cost function:

$J = \int_{0}^{\infty} (\mathbf{z}^T Q \mathbf{z} + u^T R u) \,dt$

 * **Q matrix**: A positive semi-definite matrix that penalizes deviations in the state variables from the equilibrium.
 * **R matrix**: A positive definite matrix that penalizes control effort.

The trade-off is key: large values in $Q$ lead to aggressive control, while large values in $R$ lead to conservative, energy-saving control.
"""


# ╔═╡ 2e2f90fb-98dd-46fa-a430-9dd9ff77c034
md"""
### Numerical System Model

Let's define numerical values for our system parameters and compute the numerical $A$ and $B$ matrices.
"""

# ╔═╡ 21887a7f-3734-4e82-9a22-b899c6165123
begin
	# Define a dictionary with numerical parameters
	# We use `Float64` to ensure the resulting matrices are not symbolic
	params = Dict(
		M => 1.0,      # Cart mass (kg)
	...
	# Substitute numerical values into the symbolic A and B matrices
	A_num = Float64.(substitute(A_sym, merge(params, eq_point)))
	B_num = Float64.(substitute(B_sym, merge(params, eq_point)))
	
	# Create the state-space model using ControlSystems.jl
	sys_linear = ss(A_num, B_num, I(4), 0)
end

# ╔═╡ 3ac2f8de-538f-49fb-ba30-08d4ac029a35
md"""
The system is unstable, which we can confirm by checking the eigenvalues of the $A$ matrix. A positive eigenvalue indicates instability.
"""

# ╔═╡ 0a4e1dbd-2d35-4e71-a8ef-464cbee7ae1f
eigvals(A_num)

# ╔═╡ 45e011c9-4b76-4823-a8ae-cb80c3fe98ff
md"""
## (d) 🕹️ Interactive LQR Simulation

Now for the fun part! Use the sliders below to adjust the LQR cost matrices $Q$ and $R$. Observe how the system's response changes.

 * **Position Cost ($Q_{11}$)**: How much to penalize the cart for not being at the origin ($x=0$).
 * **Angle Cost ($Q_{33}$)**: How much to penalize the pendulum for not being upright ($\theta=0$).
 * **Control Cost ($R_{11}$)**: How much to penalize using the motor force ($u$).

 We will simulate the response of both the **linearized model** and the **original nonlinear model** to the same LQR controller to see how they compare.
"""

# ╔═╡ bd8bb3db-2bf5-4aac-96b2-6c74eb73bc9a
begin
	q1_slider = @bind q1_val Slider(0.1:2:10, default=1.0, show_value=true);
	q3_slider = @bind q3_val Slider(0.1:2:10, default=10.0, show_value=true);
	r_slider = @bind r_val Slider(0.01:0.4:2.0, default=0.1, show_value=true);
	theta_slider = @bind theta_val Slider(0.1:0.1:pi/6, default=0.2, show_value=true);
end;

# ╔═╡ 7a2fc3bb-4211-408a-a326-c731deae316b
begin
	# Define Q and R matrices based on slider values
	# We penalize position (z1) and angle (z3). Velocity penalties (z2, z4) are often less critical for stabilization.
	Q = diagm([q1_val, 0, q3_val, 0])
	R = [r_val;;]

	# Compute the LQR gain K for the linear system
	# The lqr function solves the algebraic Riccati equation to find K
	K = lqr(sys_linear, Q, R)
end

# ╔═╡ f3b44be6-e629-464a-ac2f-d73be03753d7
md"""
### System Simulation

We now simulate the closed-loop system dynamics for an initial condition where the pendulum is slightly tilted.

* Initial state: $\mathbf{z}_0 = [x=0, \dot{x}=0, \theta=0.1, \dot{\theta}=0]^T$.
* Control law: $u = -K \mathbf{z}$
"""


# ╔═╡ 86d18138-78b9-4bf1-bf21-739017de7fed
begin
	# ---- 1. Simulation Setup ----
	x₀ = 0.0    # initial cart position
	θ₀ = theta_val    # initial pendulum angle (in radians, approx 5.7 degrees)
	z₀ = [x₀, 0.0, θ₀, 0.0]  # Initial state vector
	t_span = (0.0, 10.0)    # Simulation time span

	# ---- 2. Nonlinear System Dynamics ----
	# Here we build the function for the ODE solver using the symbolic expressions
	# This avoids manual transcription errors and is highly reusable.
	# The `build_function` call generates highly optimized code.
	f_nonlinear_oop = build_function(
		f_sym, 
		[z...], [u], [M, m, L, g], 
		t, 
	...
	# Define the closed-loop function for the nonlinear system
	function nonlinear_dynamics!(dz, z, p, t)
		# Unpack parameters and gain matrix
		M_val, m_val, L_val, g_val, K_mat = p
		
		# Calculate control input
		u_val = -K_mat * z
		
		# Call the generated function
		# Note: The generated function expects a scalar u, so we extract it.
		dz .= f_nonlinear_oop(z, u_val[1], [M_val, m_val, L_val, g_val], t)
	end
	
	# ---- 3. Linear System Dynamics ----
	# Define the closed-loop function for the linearized system
	function linear_dynamics!(dz, z, p, t)
		A_mat, B_mat, K_mat = p
		dz .= (A_mat - (B_mat * K_mat)) * z
	end
	
	# ---- 4. Solve the ODEs ----
	# Solve nonlinear system
	p_nonlin = (params[M], params[m], params[L], params[g], K)
	prob_nonlin = ODEProblem(nonlinear_dynamics!, z₀, t_span, p_nonlin)
	sol_nonlin = solve(prob_nonlin, Tsit5(), saveat=0.05)

	# Solve linear system
	p_lin = (A_num, B_num, K)
	prob_lin = ODEProblem(linear_dynamics!, z₀, t_span, p_lin)
	sol_lin = solve(prob_lin, Tsit5(), saveat=0.05)
end;

# ╔═╡ 2b841226-258a-4cc4-9645-6a253e498fbb
md"""
### 📊 Simulation Results

The plots below show the cart position, pendulum angle, and control force over time. The solid line represents the **nonlinear model**, while the dashed line represents the **linear model**.

The sliders below vary the $Q_{11}$ and $Q_{33}$ elements of the $Q$ matrix and the lone value in the $R$ matrix. 

The last slider at the bottom varies the initial angle $\theta_0$ to show how large deviations from equilibrium leads to inconsistencies between the linearized model and the actual nonlinear model. 
"""


# ╔═╡ 47d2b1a8-ae01-4f5c-b41f-c369098a7f72
md""" $Q_{11}$ slider: $(q1_slider)"""

# ╔═╡ 19d300a4-abc5-48de-9ea0-64d6cb46c843
 md""" $Q_{33}$ slider: $(q3_slider)"""

# ╔═╡ 5f33f5a5-1bde-4de4-a4d0-97d8cba04c89
md""" $R$ slider: $(r_slider)"""

# ╔═╡ 2d99880d-30ae-4a26-b3cc-947a29ca8d7d
md""" $\theta_0$ slider: $(theta_slider)"""

# ╔═╡ 9c65c155-b7cb-4043-8d27-3a0027dd0c65
begin
	# Plot 1: Cart Position (x)
	p1 = plot(
		sol_lin.t, [s[1] for s in sol_lin.u], 
		label="Linear", linestyle=:dash, lw=2,
		title="Cart Position", xlabel="Time (s)", ylabel="Position (m)", ylim=(0,1)
	)
	plot!(
		p1, sol_nonlin.t, [s[1] for s in sol_nonlin.u],
		label="Nonlinear", lw=2
	)
	# Plot 2: Pendulum Angle (θ)
	p2 = plot(
		sol_lin.t, [rad2deg(s[3]) for s in sol_lin.u], 
		label="Linear", linestyle=:dash, lw=2,
		title="Pendulum Angle", xlabel="Time (s)", ylabel="Angle (°)", ylim=(-15, 30)
	)
	plot!(
		p2, sol_nonlin.t, [rad2deg(s[3]) for s in sol_nonlin.u], 
		label="Nonlinear", lw=2
	)
	# Plot 3: Control Input (u)
	# u(t) = -K * z(t)
	u_lin = vcat([-K * s for s in sol_lin.u]...)
	u_nonlin = vcat([-K * s for s in sol_nonlin.u]...)
	p3 = plot(
		sol_lin.t, u_lin, 
		label="Linear", linestyle=:dash, lw=2,
		title="Control Input", xlabel="Time (s)", ylabel="Force (N)", ylim=(-10, 30)
	)
	plot!(
		p3, sol_nonlin.t, u_nonlin,
		label="Nonlinear", lw=2
	)
	plot(p1,p2,p3, layout=(3,1), size=(800,700), legend=:topright, link=:x)
end

# ╔═╡ 42cd9257-736d-4d0b-84eb-83d6705084b6
md"""
### 🤔 Interpretation

* **High Angle Cost ($Q_{33}$)**: If you increase the penalty on the angle, the controller will act more aggressively to bring $\theta$ to zero, often resulting in larger control inputs and more overshoot in the cart's position.
* **High Position Cost ($Q_{11}$)**: Increasing the penalty on position forces the cart to return to the origin more quickly.
* **High Control Cost ($R$)**: If you increase the penalty on the control force, the controller becomes "lazier" to save energy. This results in a slower response and smaller control signals. If $R$ is too high, the controller may not be strong enough to stabilize the pendulum at all.
* **Linear vs. Nonlinear ($\theta_0$)**: For small initial angles, the linear and nonlinear responses are very similar. If you were to increase the initial angle `θ₀` (last slider), you would see the linear model become a less accurate predictor of the true nonlinear behavior.
"""

# ╔═╡ 5bb7945a-f544-418c-8790-72fd684116cd
md"""## Next step

Your task in the extra credit project (if you wish to do it) is to create something similar. Pick a topic in ECE 515 (or adjacent, see below) and create an applet/simulation/demo/tool that will be useful for future students of the course. 
...
manifest_format = "2.0"
project_hash = "5664dca1417fb7fb34d2de4dbc1bad45b19ab2e8"
...
git-tree-sha1 = "fe8e17325bb5b71927a234bed4a5deeb5b06ee3a"
...
git-tree-sha1 = "18848eb5f9fe40af3c84ab2a2ae42475491110a1"
